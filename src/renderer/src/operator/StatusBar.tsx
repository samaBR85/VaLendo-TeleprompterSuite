...
export function StatusBar({ state, tab, history, dispatch, onOpenCredits }: Props): React.JSX.Element {
  ...
  const words = useMemo(
    () =>
      totalWords(
        composeLines(tab.blocks, { minWords: tab.appearance.minWords, maxWords: tab.appearance.maxWords })
      ),
    [tab.blocks, tab.appearance.minWords, tab.appearance.maxWords]
  )

  const readWords = Math.min(words, Math.max(0, wordIndexAt(state.transport, now)))
  const total = secondsForWords(words, state.transport.ppm)
  const elapsed = secondsForWords(readWords, state.transport.ppm)

  const applyTarget = (): void => {
    const seconds = parseDuration(target)
    if (seconds && words > 0) dispatch({ type: 'transport/ppm', ppm: Math.round(ppmForTarget(words, seconds)) })
    setTarget('')
  }
  ...
  return (
    <footer className="flex flex-none items-center gap-4 border-t border-[var(--color-line)] bg-[var(--color-ink-1)] px-3 py-1.5 text-[11px]">
      <Cell label="Palavras" value={words.toLocaleString('pt-BR')} />
